...
def upload_timespamp_dss(connect_to_db, dds_table, stg_orders, stg_delivery):
    dds_load.upload_timespamp(connect_to_db=connect_to_db, dds_table=dds_table, stg_orders=stg_orders, stg_delivery=stg_delivery)

def upload_users_dds(connect_to_db, dds_table, stg_table):
    dds_load.upload_users(connect_to_db=connect_to_db, dds_table=dds_table, stg_table=stg_table)

def upload_dm_products_dds(connect_to_db, dds_table, restaurant_table, stg_table):
    dds_load.upload_dm_products(connect_to_db=connect_to_db, dds_table=dds_table, restaurant_table=restaurant_table, stg_table=stg_table)

def upload_orders_dds(connect_to_db, dds_table, stg_table, time_table, user_table, restaurant_table):
    dds_load.upload_orders(connect_to_db=connect_to_db, dds_table=dds_table, stg_table=stg_table, time_table=time_table, user_table=user_table, restaurant_table=restaurant_table)

def upload_couriers_dds(connect_to_db, dds_table, stg_table):
    dds_load.upload_couriers(connect_to_db=connect_to_db, dds_table=dds_table, stg_table=stg_table)

def upload_delivery_dds(connect_to_db, dds_table, stg_table, time_table, courier_table, order_table):
    dds_load.upload_delivery(connect_to_db=connect_to_db, dds_table=dds_table, stg_table=stg_table,
...
def events_load_dds(connect_to_db, dds_table, stg_table, product_table, order_table):
    dds_load.events_load(connect_to_db=connect_to_db, dds_table=dds_table,
...
timespamp_dss = PythonOperator(task_id='timespamp_dss',
                                 ...
                                 op_kwargs={'connect_to_db': connect_to_db, 'dds_table': 'dm_timestamps', 'stg_orders': 'ordersystem_orders', 'stg_delivery':'deliveries'},
                                 dag=dag)
...
delivery_dds = PythonOperator(task_id='delivery_dds',
                                 ...
                                 op_kwargs={'connect_to_db': connect_to_db, 'dds_table': 'dm_deliveries', 'stg_table':'deliveries', 'time_table':'dm_timestamps', 'courier_table':'dm_couriers', 'order_table':'dm_orders'},
                                 dag=dag)
